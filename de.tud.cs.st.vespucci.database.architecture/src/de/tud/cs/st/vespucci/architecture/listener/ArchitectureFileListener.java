...
public class ArchitectureFileListener implements IArchitectureObserver {
	...
	public ArchitectureFileListener(UnissonDatabase database,
	...
	private boolean isRegisteredModel(IResource resource) {
		return isGlobalModel(resource) || isModel(resource);
	}

	private boolean isGlobalModel(IResource resource) {
		return resource.getFullPath().toString().equals(globalModel);
	}

	private boolean isModel(IResource resource) {
