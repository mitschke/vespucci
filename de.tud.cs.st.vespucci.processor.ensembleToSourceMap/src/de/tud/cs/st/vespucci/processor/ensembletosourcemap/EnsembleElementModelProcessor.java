...
public class EnsembleElementModelProcessor implements IModelProcessor {
	...
	public Object processModel(Object diagramModel) {
		...
		for (final IEnsemble ensemble : model.getEnsembles()) {
			...
			try {
				...
				reportParserError(diagramFile, e.getMessage());
				continue;
			}
		...
		if (mappings.size() == 1) {
			return new EnsembleElementListView(mappings.get(0));
		}
		...
		return new EnsembleElementListView(union);
	}

	private void reportParserError(IFile diagramFile, String msg) {
		try {
	...
	private UnissonQuery getQuery(IEnsemble ensemble)
		...
		if (result.$outer instanceof Parsers.Failure) {
			...
			throw new UnissonParserException(failure.msg().substring(0,
