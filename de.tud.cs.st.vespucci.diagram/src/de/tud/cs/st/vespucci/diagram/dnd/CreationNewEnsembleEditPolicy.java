...
public final class CreationNewEnsembleEditPolicy extends CreationEditPolicy {
	...
	class ExtendedSetValueCommand extends SetValueCommand {
		...
		/**
		 * Sets the {@link #createRequest} additionally to
		 * {@link SetValueCommand#SetValueCommand(SetRequest)}.
		 * 
		 * @param request
		 * @param createRequest
		 */
		protected ExtendedSetValueCommand(final SetRequest request,
				final CreateElementRequest createRequest) {
			super(request);
	...
	private ExtendedSetValueCommand createSetNameCommand(
			final CreateElementRequest createElementRequest,
			final CreateViewAndElementRequest request) {
		final SetRequest setNameRequest = new SetRequest(
	...
	private ExtendedSetValueCommand createSetQueryCommand(
			final CreateElementRequest createElementRequest,
			final Request request) {
		@SuppressWarnings("unchecked")
		final SetRequest setQueryRequest = new SetRequest(
				createElementRequest.getEditingDomain(),
				createElementRequest.getNewElement(),
				vesPackage.getShape_Query(),
				QueryBuilder.createQueryFromRequestData(request
						.getExtendedData()));
		return new ExtendedSetValueCommand(setQueryRequest,
				createElementRequest);
	}

	/**
	 * @return Returns a create command if the request is understood i.e. its
	 *         type is REQ_DROPNEWENSEMBLE.
	 */
	@Override
	 ...
	 * {@link CreationEditPolicy#getCreateElementAndViewcommand}. This version
	 * should only be used in this class. This method returns a CompositeCommand
	 * that include - a create view element command create semantic element
	 * command - two setValue commands
	 * 
	 * @param request
	 * @return Command that creates the semantic and the view command for the
	 *         given CreateViewAndElementRequest
	 */
	private Command createElementAndViewEnsembleCommand(
			final CreateViewAndElementRequest request) {

		// copied content
		// get the element descriptor
		final CreateElementRequestAdapter requestAdapter = request
		...
		cc.compose(new SemanticCreateCommand(requestAdapter,
		...
		// create two SetValueCommand to set the Query and the Name of the new
		// ensemble
