...
class SADServerTest extends FlatSpec with ShouldMatchers with BeforeAndAfterAll {
  ...
  "TransactionalDescriptionResource" should "allow update of name in stored SAD" in {
    val response = get(host + descriptionCollectionPath + "/" + id1)
    val description = Description(response.body)
    val oldName = description.name
    description.name = "someNewName"
    println("DESC:" + description.toXML.toString);
    val tUrl = new Transaction(beginTransaction((<id>{id1}</id>).toString).body).url
    Description(get(host + descriptionCollectionPath + "/" + id1).body).name should equal { oldName }
    println("URL: " + tUrl)
    authPut(tUrl, Entity(description.toXML.toString, "application/xml", "UTF-8")) // update
    authPost(tUrl, Entity(<whatever/>.toString, "application/xml", "UTF-8")) // commit
    Description(get(host + descriptionCollectionPath + "/" + id1).body).name should equal { "someNewName" }
  }
